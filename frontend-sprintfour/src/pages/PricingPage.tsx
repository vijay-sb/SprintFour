  ...
  return (
    <div className="min-h-screen">
      <div className="mx-auto max-w-5xl px-6 pb-12 pt-16 text-center">
        <h1 className="mb-3 text-4xl font-bold text-neutral-900">Choose your speed</h1>
      ...
      <div className="mx-auto max-w-4xl px-6 pb-20">
          ...
          >
            ...
            <div className="mt-6 border-t border-neutral-200 pt-4 text-center">
              ...
              <div className="flex items-center justify-center gap-3">
                <div>
                  <div className="font-mono text-lg font-bold text-neutral-700">3s</div>
                  <div className="text-[10px] text-neutral-400">per doc</div>
                </div>
                <div className="text-neutral-400">vs</div>
                <div>
                  <div className="font-mono text-lg font-bold text-red-500 line-through opacity-70">
                    45s
                  </div>
                  <div className="text-[10px] text-neutral-400">manual</div>
                </div>
              </div>
            </div>
          </PlanCard>
        ...
        <div className="mt-16">
          <h3 className="mb-2 text-center text-lg font-bold text-neutral-900">Why Pro is faster — measured</h3>
          <p className="mx-auto mb-8 max-w-xl text-center text-xs text-neutral-500">
        ...
        {/* Comparison */}
        <div className="mt-16">
          <h3 className="mb-8 text-center text-lg font-bold text-neutral-900">Feature comparison</h3>
          <div className="overflow-hidden rounded-xl border border-neutral-200">
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-neutral-200 bg-white">
                  <th className="px-4 py-3 text-left font-medium text-neutral-500">Feature</th>
                  <th className="px-4 py-3 text-center font-medium text-neutral-500">Free</th>
                  <th className="px-4 py-3 text-center font-medium text-neutral-700">Pro</th>
                </tr>
              </thead>
              <tbody>
                {COMPARISON.map(([feature, free, pro]) => (
                  <tr key={feature} className="border-b border-neutral-200 hover:bg-white">
                    <td className="px-4 py-3 text-neutral-600">{feature}</td>
                    <td className="px-4 py-3 text-center text-neutral-400">{free}</td>
                    <td className="px-4 py-3 text-center text-neutral-700">{pro}</td>
                  </tr>
                ))}
              </tbody>
            </table>
...
function PlanCard({
  ...
  return (
    <div className={`relative rounded-xl border bg-white p-8 transition ${ring}`}>
      {badge ? (
        <div className="absolute right-0 top-0 rounded-bl-lg rounded-tr-xl bg-neutral-900 px-3 py-1 text-[10px] font-bold text-white">
          {badge}
        </div>
      ) : null}
      {active ? (
        <div
          className={`mb-4 text-[10px] font-bold uppercase tracking-wider ${
            accent === "cyan" ? "text-neutral-700" : "text-neutral-500"
          }`}
        >
          Current plan
        </div>
      ...
      <p className="mb-6 text-xs text-neutral-400">{blurb}</p>
      {children}
    </div>
  );
}
      ...
      {items.map((item) => (
        <li
          ...
          className={`flex items-center gap-2 text-xs ${
            !item.on ? "text-neutral-400" : item.strong ? "font-medium text-neutral-900" : "text-neutral-600"
          }`}
        >
          <span
            className={`flex h-4 w-4 items-center justify-center rounded-full text-[10px] ${
              item.on ? "bg-emerald-50 text-emerald-700" : "bg-neutral-100 text-neutral-400"
            }`}
          >
            {item.on ? "✓" : "—"}
          </span>
          {item.text}
          {item.strong ? (
            <span className="rounded bg-neutral-100 px-1.5 py-0.5 text-[9px] font-bold text-neutral-700">
              PRO
            </span>
          ) : null}
        </li>
...
function PlanButton({
  ...
  const styles = disabled
    ...
    : tone === "primary"
      ...
      : "border border-neutral-200 bg-neutral-100 text-neutral-600 hover:bg-neutral-100";
